...
def main():
	...
	SGD = StochasticGradientDescent(learning_rate = 0.001, momentum = 0.9, nesterov = True)
	...
	train_err, val_err = model.fit(X_train,y_train, n_epochs = 1000, batch_size=256)

	y_train_pred = model.predict(X_train)
	print("===Training results===")
	print("R-square on y1",R_square(y_train_pred[0], y_train[0]))
	print("Overal error on traning set",(mean_squared_error(y_train_pred[0], y_train[0]))/2)
	y_val_pred = model.predict(X_val)
	print("===Validation results===")
	print("R-square on y1", R_square(y_val_pred[0], y_val[0]))
	print("Overal error on valiation set",(mean_squared_error(y_val_pred[0], y_val[0]))/2)
	...
	print("Overal blind test error", (mean_squared_error(y_pred[0], y_test[0]))/2 )
